...
try {
    docker push "$env:REGISTRY/workbench-quint:staging"
    if ($LASTEXITCODE -ne 0) {
        throw "Docker push failed with exit code $LASTEXITCODE"
    }
    Write-Host "Docker image pushed successfully to $env:REGISTRY/workbench-quint:staging" -ForegroundColor Green
} catch {
