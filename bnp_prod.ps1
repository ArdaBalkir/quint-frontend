...
try {
    docker build . -t "$env:REGISTRY/workbench-quint:production"
...
try {
    docker push "$env:REGISTRY/workbench-quint:production"
    if ($LASTEXITCODE -ne 0) {
        throw "Docker push failed with exit code $LASTEXITCODE"
    }
    Write-Host "Docker image pushed successfully to $env:REGISTRY/workbench-quint:production" -ForegroundColor Green
} catch {
